...
for sigma_p in sigma_p_list:
    iii = iii + 1
    g_list_sus_tmp = np.load('python/data/sigma_p_study_sus/sus_example_1_d10_N1000_Nsim100_mmh_sigmap' \
        + repr(iii) + '_g_list.npy')
    g_list_list_sus.append(g_list_sus_tmp)

iii = 0
for sigma_p in sigma_p_list:
    iii = iii + 1
    g_list_mp_tmp = np.load('python/data/sigma_p_study/mp_example_1_d10_N100_Nsim100_b5_mmh_sss2_sigmap' \
        + repr(iii) + '_g_list.npy')
...
plt.plot(sigma_p_list, cov_at_pf_array_mp,'+-', label=r'MP with MMH', color='C1')
# plt.plot(sigma_p_list, cov_at_pf_array_sus,'x-', label=r'SuS with MMH', color='C2')
...
plt.tight_layout()
if savepdf:
    plt.savefig('sigma_p_study_cov_over_b_mmh.pdf', format='pdf', dpi=50, bbox_inches='tight')

# plot pf over d

plt.figure()
plt.plot(sigma_p_list, pf_analytical,'o-', label=r'analytical', color='C0')
plt.plot(sigma_p_list, pf_mean_array_mp,'+-', label=r'MP with MMH', color='C1')
# plt.plot(sigma_p_list, pf_mean_array_sus,'x-', label=r'SuS with MMH', color='C2')
...
plt.tight_layout()
if savepdf:
    plt.savefig('sigma_p_study_pf_over_b_mmh.pdf', format='pdf', dpi=50, bbox_inches='tight')
