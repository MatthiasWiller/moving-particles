...
for sigma_p in sigma_p_list:
    iii = iii + 1
    g_list_mp_tmp = np.load(direction + 'mp_example_1_d10_N100_Nsim100_b5_cs_sss2_sigmap' \
...
for sigma_p in sigma_p_list:
    iii = iii + 1
    g_list_mp_tmp = np.load(direction + 'mp_example_1_d10_N100_Nsim100_b5_mmh_sss2_sigmap' \
        + repr(iii) + '_g_list.npy')
...
for i in range(0, n_sigma):
    b_line_mp, pf_line_list_mp        = uutil.get_pf_line_and_b_line_from_MP(g_list_list_cs[i], n_initial_samples)
...
for i in range(0, n_sigma):
    b_line_mp, pf_line_list_mp        = uutil.get_pf_line_and_b_line_from_MP(g_list_list_mmh[i], n_initial_samples)
    pf_line_mean_mp, pf_line_cov_mp   = uutil.get_mean_and_cov_from_pf_lines(pf_line_list_mp)
    ...
    cov_line_list.append(pf_line_cov_mp)
    legend_list.append(r'MP')
    cov_at_pf_array_mmh[i] = pf_line_cov_mp[0]
    pf_mean_array_mmh[i] = pf_line_mean_mp[0]
...
plt.plot(sigma_p_list, cov_at_pf_array_cs,'x', label=r'MP with CS')
plt.plot(sigma_p_list, cov_at_pf_array_mmh,'+', label=r'MP with MMH')
